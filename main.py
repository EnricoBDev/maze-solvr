    ...
    if filename_open_path:
        ...
        if height < 300 and width < 300:
            preview_Image = preview_Image.resize(
                (width * 3, height * 3), resample=Image.Resampling.NEAREST)
        ...
        ttk.Label(top_popup, text="Is this the image you wanted?").grid(
    ...
    if maze:
        ...
        if not path:
            messagebox.showerror(
                title="Error", message="The maze you chose is not solvable")
        else:
            filetypes = (("Image files", "*.png"), ("All files", "*.*"))
            ...
            if is_gif_created.get():
                frames = create_full_gif(
                    start_img, final_Image, solved_frames, execution_frames)
                create_gif(gif_save_path, frames,
                           gif_speed.get(), is_gif_loop.get())
        ...
        messagebox.showerror(
            title="Error", message="There are too many/too little entry/finish points")
...
if platform.system() == "Windows":
    ...
    root.iconbitmap("@assets/icons/maze.xbm")
...
generate_gif_checkbox = ttk.Checkbutton(
    root, text="Generate gif", variable=is_gif_created, onvalue=True, offvalue=False, command=enable_loop)
generate_gif_checkbox.grid(column=2, row=0, padx=(5, 0), pady=(10, 0))

loop_checkbox = ttk.Checkbutton(
    root, text="Loop gif", variable=is_gif_loop, onvalue=0, offvalue=1, state=DISABLED)
loop_checkbox.grid(column=2, row=1, padx=(5, 0))

slider = ttk.Scale(root, from_=1, to=5, orient="horizontal",
                   variable=gif_speed, command=slider_change)
